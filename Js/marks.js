firebase.auth().onAuthStateChanged((user) => {
    if (user) {
        ...
        firebase.firestore().collection("users").doc(UserId).get().then((doc) => {
            ...
            //ordering students according to performance per subjects
        document.getElementById("OrderSubjects").onclick = function() {
            let selectedSubject = document.getElementById("selectSubjects").value;
            document.getElementById("marksDisplay").style.display = "none";
            document.getElementById("SpecificSubjects").style.display = "block";
            if (selectedSubject == "English") {
                console.log(selectedSubject)
                firebase.firestore().collection("marks").orderBy("Eng", "desc").get().then((virtualSnapshot) => {
                    let content = '';
                    let position = 1;
                    ...
                    $("#SpecificSubjects").append(content);
